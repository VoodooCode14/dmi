...
def run_abs_dmi(low_freq_data, high_freq_data,
...
def run_hilbert_dmi(low_freq_data, high_freq_data,
         phase_window_half_size = 10, phase_step_width = 1,
         max_model_fit_iterations = 200):
    """
    Calculates the direct modulation index between a low frequency signal and a high frequency signal. Instead of the original modulation index based on entropy, this modulation index estimate is based on a sinusoidal fit. 
...
def get_data(frequency_sampling = 1000, frequencies_between_bursts = [2, 5, 10, 15, 20, 25, 30], 
    ...
    for _ in range(rep_cnt):
        ...
        for low_freq_signal in low_freq_signals:
            ...
            dmi_abs_score = run_abs_dmi(low_freq_signal, high_freq_signal, phase_window_half_size = 4, phase_step_width = 2)[0]
            dmi_hilbert_score = run_hilbert_dmi(low_freq_signal, high_freq_signal, phase_window_half_size = 4, phase_step_width = 2)[0]
            np.random.seed(0)
            mi_abs_score = run_abs_mi(low_freq_signal, high_freq_signal) * 100
            np.random.seed(0)
            mi_hilbert_score = run_hilbert_mi(low_freq_signal, high_freq_signal) * 100
            
            loc_scores = [dmi_abs_score, dmi_hilbert_score, mi_abs_score, mi_hilbert_score]
...
def main():
    ...
    plt.plot(scores[:, 0], color = "green", linewidth = 3)
    plt.plot(scores[:, 1], color = "blue", linestyle='dashed', linewidth = 3)
    plt.plot(scores[:, 2], color = "red", linewidth = 3)
    plt.plot(scores[:, 3], color = "purple", linestyle='dashed', linewidth = 3)
    plt.show(block = True)
